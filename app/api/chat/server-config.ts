...
declare global {
  namespace NodeJS {
    interface ProcessEnv {
      ...
      // chat session management prompts
      SESSION_TITLE_PROMPT?: string;
      SESSION_MEMORY_PROMPT?: string;
...
const ACCESS_CODES = (function getAccessCodes(): Set<string> {
  ...
  try {
    const codes = (code?.split(",") ?? [])
      .filter((v) => !!v)
      .map((v) => {
        // Simple hash function replacement for spark-md5
        let hash = 0;
        if (v.length === 0) return hash.toString();
...
function isGPT4Model(modelName: string): boolean {
